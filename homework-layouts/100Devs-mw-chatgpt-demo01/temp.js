...
app.get("/admin", async (req, res) => {
  try {
    ...
    const serviceRequests = await FormData.find().sort({ date: 1, time: 1 });
...
app.post("/servicerequest", async (req, res) => {
  try {
    // Create a new FormData object from the service request form data
    const serviceRequest = new FormData({
      name: req.body.name,
